...
- you need to change `min_size = 0` and `max_size = 0` parameters of `aws_autoscaling_group example_blue` resources.  
- this way the size of the autoscaling group for blue is kept to 0 and no traffic goes to it.
- you need to keep `min_size = 1` and `max_size = 1` parameters of `aws_autoscaling_group example_green` resources.  
